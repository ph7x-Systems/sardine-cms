...
| Design-system vocabulary (`_components.html.j2`, ADR-0055) | `page_header` (one h1 + the screen's single primary action), `editor_header` (back link + identity h1 + status beside it), `action_bar`, `editor_nav` ("On this page" anchors for long editors), `danger_zone` (bordered, always last), `empty_state` (meaning + next action), `disclosure` (native `details`, closed by default), `filter_bar` (one GET toolbar per list), `summary_line` + `pagination` (the DataTable contract, DS-19), `form_section` (one card per form area) — born on the menu golden screen, adopted by the content lists, editors and settings screens |
