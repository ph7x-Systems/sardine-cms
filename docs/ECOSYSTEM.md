...
| Deployment target | `Target` protocol + `register_target` shapes the artifact for a host; deployment providers transport and activate it — see [DEPLOYMENT_PROVIDERS.md](DEPLOYMENT_PROVIDERS.md) and [WRITING_A_DEPLOYMENT_PROVIDER.md](WRITING_A_DEPLOYMENT_PROVIDER.md) | Adapter tests over a built artifact; deployment conformance suite |
