# Support

- **Questions and ideas** — open a
  [GitHub issue](https://github.com/ph7x-Systems/stillsite/issues)
  with the question template. Please search existing issues first.
- **Bugs** — open an issue with the bug template: steps to reproduce, expected
  vs. actual behavior, versions.
- **Security vulnerabilities** — never open a public issue; follow
  [SECURITY.md](SECURITY.md).
- **Contributing** — read [CONTRIBUTING.md](CONTRIBUTING.md) first; pull
  requests are welcome once an issue has been discussed.

This project is maintained by pH7x Systems. Response times are best-effort.
